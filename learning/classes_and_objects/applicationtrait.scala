/*
"To use the trait, you first write “ extends Application ” after the name
...
cation just like any other.

The way this works is that trait Application declares a main method
...
you need to access command-line arguments, because the args array isn’t
available. For example, because the Summer application uses command-line
arguments, it must be written with an explicit main method, as shown in List-
ing 4.3. Second, because of some restrictions in the JVM threading model,
...
an object which is executed by the Application trait. So you should in-
herit from Application only when your program is relatively simple and
single-threaded.
